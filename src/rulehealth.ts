// 规则体检：哪条太宽（误伤源头）、哪条根本没在干活（写错了/过时了）。
// 计数必须持久化——「从没命中」是长期结论，会话日志只有 300 条、刷新即清，判不出来。
//
// 判死规则的四条自我约束（宁可少报，不能冤枉一条正在生效的规则）：
//   1. 统计不满 OBSERVE_DAYS 一律不报（刚装上的人所有规则都还没命中）
//   2. 联网维度在「精确过滤」关着时压根不会被求值 → 归「未启用」
//   3. 只报用户自己名单里的规则（订阅里的删不掉，报了干着急）
//   4. 用户自己停用的单独归类（它零命中是被按下去的）
